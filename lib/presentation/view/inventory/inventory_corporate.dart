...
class InventoryCorporateScreen extends StatefulWidget {
  static const String routeName = '/inventory/corporate';

  const InventoryCorporateScreen({super.key});

  @override
  State<InventoryCorporateScreen> createState() =>
      _InventoryCorporateScreenState();
}

class _InventoryCorporateScreenState extends State<InventoryCorporateScreen> {
  final box = GetStorage();
  final userData = GetStorage().read('userData');
  int selectedFilterIndex = 0;

  final InventoryCorporateController _inventoryCorporateController = Get.find();
  ...
  void didChangeDependencies() {
    ...
    if (userData != null) {
      _inventoryCorporateController.getInventoryList(
        idUser: userData['id'].toString(),
      );
    }
  }

  List<InventoryCorporateModel> _filterData(
      List<InventoryCorporateModel> inventoryList) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Center(
        child: Obx(
          () {
            var inventoryCorporateList =
                _inventoryCorporateController.inventoryCorporateList;
            List<InventoryCorporateModel> inventoryWithFilter =
                _filterData(inventoryCorporateList);
                                                  ...
                                                  InkWell(
                                                    onTap: () {
                                                      Clipboard.setData(
                                                          ClipboardData(
                                                        text:
                                                            "https://talentdna-dev.meteor.asia/main-screen?claim=${inventoryWithFilter[index].link}&event-id=${inventoryWithFilter[index].event_id}&type=${inventoryWithFilter[index].type}",
                                                      ));
